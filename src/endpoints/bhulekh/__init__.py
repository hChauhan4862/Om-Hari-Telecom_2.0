...
async def village_search(search_text: str = Path(...,min_length=1), db: Session = Depends(get_db)):
    return hcBhulekh(db).village_search_json(search_text)

@router.get("/search_village/{search_text}/{offset}")
async def village_search_info(search_text: str = Path(...,min_length=2), offset: int = Path(ge=0), db: Session = Depends(get_db)):
    return hcBhulekh(db).village_search_json(search_text, offset)

@router.get("/village/{village_code}")
async def village_info_view(village_code: int = Path(le=999999,ge=100000), db: Session = Depends(get_db)):
    return hcBhulekh(db).village_search_json(village_code)

@router.get("/search_gata/{village_code}/{gata_no}")
async def gata_wise_search(village_code: int = Path(le=999999,ge=100000), gata_no: str = Path(), db: Session = Depends(get_db)):
    return hcBhulekh(db).list_search(village_code=village_code, act="sgw", value=gata_no)

@router.get("/search_khata/{village_code}/{khata_no}")
async def khata_wise_search(village_code: int = Path(le=999999,ge=100000), khata_no: int = Path(gt=0), db: Session = Depends(get_db)):
    khata_no = str(khata_no).zfill(5)
    return hcBhulekh(db).list_search(village_code=village_code, act="sbacn", value=khata_no)

@router.get("/search_name/{village_code}/{name}")
async def name_wise_search(village_code: int = Path(le=999999,ge=100000), name: str = Path(), db: Session = Depends(get_db)):
    return hcBhulekh(db).list_search(village_code=village_code, act="sbname", value=name)


@router.get("/khata/{village_code}/{khata_no}")
async def get_khata(village_code: int = Path(le=999999,ge=100000), khata_no: int = Path(...,le=99999), db: Session = Depends(get_db)):
    return hcBhulekh(db).khata_json(khata_no, village_code)

@router.get("/ansh/{village_code}/{khata_no}")
async def get_khata_ansh(village_code: int = Path(le=999999,ge=100000), khata_no: int = Path(...,le=99999), db: Session = Depends(get_db)):
    return hcBhulekh(db).ansh_json(khata_no, village_code)

@router.get("/updateVillageList")
async def update_villages_list(background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
